...
const currentRasterList = (
  state: MyStore['currentRasterList'] = null,
  action: Action
): MyStore['currentRasterList'] => {
  switch (action.type) {
...
const currentWMSList = (
  state: MyStore['currentWMSList'] = null,
  action: Action
): MyStore['currentWMSList'] => {
  switch (action.type) {
    case REQUEST_WMS:
      return {
        count: 0,
...
const observationTypes = (state: MyStore['observationTypes'] = [], action: Action): MyStore['observationTypes'] => {
    switch (action.type) {
        case OBSERVATION_TYPES_FETCHED:
            ...
            //Update Redux state
        return filteredObservationTypes.map((observation: ObservationType) => {
                return {
                    url: observation.url,
...
const organisations = (state: MyStore['organisations'] = [], action: Action): MyStore['organisations'] => {
    switch (action.type) {
        case ORGANISATIONS_FETCHED:
            ...
            const organisationsWithoutEmptyNames = action.organisations.filter((organisation: Organisation) => organisation.name !== '');
            ...
            //Update Redux state
            return filteredOrganisations.map((organisation: Organisation) => {
                return {
...
const datasets = (state: MyStore['datasets'] = [], action: Action): MyStore['datasets'] => {
    switch (action.type) {
